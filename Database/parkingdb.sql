...
--
-- Temporary view structure for view `posti_parcheggio_liberi_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `posti_parcheggio_liberi_view` AS select `liberi`.`id_parcheggio` AS `id_parcheggio`,`liberi`.`id_tipo` AS `id_tipo`,(`liberi`.`numero_posti` - ifnull(`occupati`.`postiOccupati`,0)) AS `postiLiberi` from (((select `parkingdb`.`posti_parcheggio`.`id_parcheggio` AS `id_parcheggio`,`parkingdb`.`posti_parcheggio`.`id_tipo` AS `id_tipo`,`parkingdb`.`posti_parcheggio`.`numero_posti` AS `numero_posti` from (`parkingdb`.`posti_parcheggio` join `parkingdb`.`tipo_parcheggio`) where (`parkingdb`.`tipo_parcheggio`.`idTipo_parcheggio` = `parkingdb`.`posti_parcheggio`.`id_tipo`))) `liberi` left join (select `parkingdb`.`prenotazioni_in_atto`.`id_parcheggio` AS `parcheggioID`,`parkingdb`.`prenotazioni_in_atto`.`id_tipo_posto` AS `tipoID`,count(0) AS `postiOccupati` from `parkingdb`.`prenotazioni_in_atto` group by `parkingdb`.`prenotazioni_in_atto`.`id_parcheggio`,`parkingdb`.`prenotazioni_in_atto`.`id_tipo_posto`) `occupati` on(((`liberi`.`id_parcheggio` = `occupati`.`parcheggioID`) and (`liberi`.`id_tipo` = `occupati`.`tipoID`)))) order by `liberi`.`id_parcheggio`,`liberi`.`id_tipo` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
